...
def generate(
    ...
    header_lines.extend([
        ...
        "    .data = {0},",
        "};",
        "static struct WaveData *sLastUnknownMusicSample;",
        "",
        ...
        "        u64 resourceSize = 0;",
        "",
        "        // Instruments inside already-resolved voicegroups can pass the cached",
        "        // resource pointer through this function again. Keep resolution idempotent",
        "        // so a valid sample is not mistaken for an unknown compiled identifier.",
        ...
        "    }",
        "",
        "    // Cries and other WaveData resources are also cached by the pack but do",
        "    // not belong to the generated music-sample table. Accept an exact cached",
        "    // allocation only after validating its WaveData header. Compressed cries",
        "    // store the decoded sample count in size, which can exceed the packed payload.",
        "    {",
        "        u64 resourceSize = 0;",
        "        if (ResourcePack_GetCachedSize(compiledData, &resourceSize)",
        "         && resourceSize > offsetof(struct WaveData, data)",
        "         && compiledData->freq != 0",
        "         && compiledData->size != 0)",
        "            return compiledData;",
        "    }",
        "",
        "    if (compiledData != NULL && compiledData != sLastUnknownMusicSample)",
        "    {",
        "        DBGPRINTF(\"Music sample: unknown identifier %p; using silence instead of dereferencing it\\n\", compiledData);",
        "        sLastUnknownMusicSample = compiledData;",
        "    }",
        "    return (struct WaveData *)&sMissingMusicSample;",
        "}",
        "",
