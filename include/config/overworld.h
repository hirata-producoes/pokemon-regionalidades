...
#define OW_DEFOG_FIELD_MOVE             TRUE       // Fog can affect traversal and be cleared by the player.
...
#define OW_BATTLE_ONLY_FORMS           FALSE      // Requires OW_POKEMON_OBJECT_EVENTS and is unused while followers/visible wild Pokémon are disabled.
...
#define TIME_REGIONALIDADES             GEN_COUNT + 4

// Time
#define OW_TIMES_OF_DAY                 GEN_LATEST   // Different generations have the times of day change at different times.
#define OW_USE_FAKE_RTC                 TRUE         // The persisted world clock advances during play; Regionalidades also reconciles elapsed real time when loading.
#define OW_ALTERED_TIME_RATIO           TIME_REGIONALIDADES // Pokémon Regionalidades uses the 3x World Clock specified by decision D-023.
#define OW_TIME_OF_DAY_ENCOUNTERS       TRUE         // Morning, day and night can use different encounter tables.
...
// Map pop-up config
#define OW_POPUP_GENERATION        GEN_5    // World-map pop-ups can display the current in-game time.
...
#define OW_POPUP_BW_TIME_MODE      OW_POPUP_BW_TIME_24_HR   // Show the current world time when entering an area.
