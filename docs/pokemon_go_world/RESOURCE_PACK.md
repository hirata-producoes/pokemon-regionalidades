...
Os 12 recursos usados pela tela de título Emerald, 2.950 imagens, 2.889 paletas, 1.420 ícones, 1.031 pegadas, 1.067 cries de Pokémon, 306 recursos de tileset, 174 frames de animação de tileset, 106 amostras musicais, 530 faixas e 195 voicegroups MP2K foram removidos do executável PC. Os tilesets abrangem 85 gráficos de tiles, 77 conjuntos de paletas, 72 tabelas de metatiles e 72 tabelas de atributos; as amostras musicais atendem 156 identificadores, incluindo aliases de fonemas compartilhados. Os geradores inspecionam as configurações ativas com o pré-processador C, preservando formas habilitadas, diferenças de gênero e escolhas entre recursos modernos/GBA. Suas definições compiladas permanecem no alvo GBA.

As faixas usam o contêiner interno `PGWSONG` versão 1. O gerador lê os objetos COFF produzidos por `mid2agb`, guarda os dados de track e uma tabela de relocações, e deixa no executável somente um símbolo-placeholder de 8 bytes por música. Ao iniciar uma música, o PC carrega o recurso, reconstrói ponteiros internos de `GOTO`/`PATT` e resolve a voicegroup externa correspondente. Uma faixa ausente ou inválida vira uma música vazia segura; músicas dinâmicas usadas pelos cries continuam aceitas sem conversão.

As voicegroups usam o contêiner interno `PGWVOICE` versão 1. Cada recurso contém sua tabela de instrumentos e relocações de 64 bits para dependências entre voicegroups, keysplits, ondas programáveis e placeholders de amostras. Drumsets que começam em uma nota diferente de zero preservam o recuo do label e todos os índices MIDI válidos. Recursos ausentes ou inválidos usam uma tabela completa de instrumentos CGB seguros, impedindo acesso fora dos limites.
