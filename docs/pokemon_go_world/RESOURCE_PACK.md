...
1. Adicione uma entrada com nome único e `source` em `resources/pc/manifest.json`. Use `sources` para concatenar vários arquivos, na ordem informada, em uma única entrada.
   Famílias geradas podem ser referenciadas por `resource_lists`; a lista de imagens, paletas, ícones e pegadas de Pokémon é criada em `build/pc-generated/pokemon_resources.json`.
