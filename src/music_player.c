...
// Native MP2K sequencer adapted from gradenGnostic/pokeemerald-multiplatform.
...
STATIC_ASSERT(sizeof(struct ToneData) == 0xC, NativeToneDataSize);
STATIC_ASSERT(sizeof(struct MusicPlayerTrack) == 0x50, NativeMusicPlayerTrackSize);
STATIC_ASSERT(sizeof(struct SoundChannel) == 0x40, NativeSoundChannelSize);
STATIC_ASSERT(sizeof(struct CgbChannel) == sizeof(struct SoundChannel), NativeCgbChannelSize);

// Don't uncomment this. vvvvv
// #define POKEMON_EXTENSIONS
#define MIXED_AUDIO_BUFFER_SIZE 4907

extern u32 MidiKeyToFreq(struct WaveData2 *wav, u8 key, u8 pitch);
...
void MP2KClearChain(void *channel) {
    struct MixerSource *chan = channel;
...
void MP2K_event_nxx(u32 clock, struct MP2KPlayerState *player, struct MP2KTrack *track) { // ply_note
    struct SoundMixerState *mixer = (struct SoundMixerState *)SOUND_INFO_PTR;
    ...
    if (cgbType != 0) {
        ...
        chan = (struct MixerSource *)mixer->cgbChans + cgbType - 1;
